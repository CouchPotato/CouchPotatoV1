...
class configApp():
    ...
    def initConfig(self):
        ...
        if not self.p.has_section('handler_accesslog'):
            self.addSection('handler_accesslog')
            self.setDefault('handler_accesslog', 'formatter', 'generic')
            self.setDefault('handler_accesslog', 'class', 'handlers.RotatingFileHandler')
            self.setDefault('handler_accesslog', 'args', "('logs/MovieManager.log','a', 10000, 4)")
            self.setDefault('handler_accesslog', 'level', 'INFO')

            self.addSection('handlers')
            self.set('handlers', 'keys', 'console, accesslog')
            
            self.addSection('logger_root')
            self.set('logger_root', 'handlers', 'console, accesslog')
