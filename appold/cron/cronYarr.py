...
class YarrCron(cronBase, rss):
    ...
    def run(self):
        ...
        if not Environment.doDebug():
            time.sleep(10)

        wait = 0.1 if Environment.doDebug() else 1
        while True and not self.abort:
            ...
            #check all movies
            now = time.time()
            if (self.lastChecked + self.intervalSec) < now:# and not Environment.doDebug():
    ...
    def _search(self, movie, force = False):
        ...
        for queue in movie.queue:
            ...
            if queue.active and (queue.lastCheck < (now - 300) or Environment.doDebug() or force) and not queue.completed and not self.abort and not self.stop:
