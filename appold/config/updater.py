...
class Updater(SimplePlugin):
    ...
    url = 'http://github.com/RuudBurger/Environment/tarball/master'
    downloads = 'http://github.com/RuudBurger/Environment/downloads'
    timeout = 10
    running = False
    version = None
    ...
    def getVersion(self, force = False):
        ...
        if not self.version or force:
            if self.isFrozen:
                ...
                handle.close()
                self.version = lineList[-1].replace('RuudBurger-Environment-', '').replace('.tar.gz', '')
    ...
    def doUpdateUnix(self):
        ...
        log.info('Moving updated files to Environment root.')
